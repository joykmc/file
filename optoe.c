...
static struct device *printk_dev = NULL; /* limit printk info */
/*
				...
				size_t count, int opcode)
{
	...
	if (page > 0) {
		...
		ret = optoe_eeprom_write(optoe, client, &page,
			OPTOE_PAGE_SELECT_REG, 1);
		if (ret < 0) {
			/* [ieit] the error log of the same device is printk only once */
			if(printk_dev != &client->dev) {
				printk_dev = &client->dev;
				if(printk_ratelimit())
					dev_err(&client->dev,
...
static int optoe_remove(struct i2c_client *client)
{
	...
	sysfs_remove_group(&client->dev.kobj, &optoe->txdis_attr_group);
...
static ssize_t show_eeprom_txdis(struct device *dev,
			struct device_attribute *dattr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct optoe_data *optoe = i2c_get_clientdata(client);
			...
			const struct i2c_device_id *id)
{
	int err;
	int use_smbus = 0;
	struct optoe_platform_data chip={0};
	struct optoe_data *optoe;
	int num_addresses = 0;
	char port_name[MAX_PORT_NAME_LEN];
	...
	if (optoe->dev_class == ONE_ADDR) {
		optoe->txdis_attr_group = txdis_attr_group;
		err = sysfs_create_group(&client->dev.kobj, &optoe->txdis_attr_group);
		if (err) {
			if(printk_ratelimit())
				dev_err(&client->dev, "failed to create txdis sysfs attribute group.\n");
			goto err_struct;
		}
	}
